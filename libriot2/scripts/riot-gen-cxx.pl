...
# ldplfs mpicxx script to mimic it's behavior
...
$FUNCTION_WRAPPERS="-Wl,-wrap,write,-wrap,open,-wrap,open64,-wrap,close,-wrap,read,-wrap,pread,-wrap,pwrite,-wrap,pread64,-wrap,pwrite64,-wrap,fcntl";
...
my $version_out = `$input_file -v 2>&1 |head -n 1`;
if (!($version_out))
{
...
$link_cmd =~ s/foo.o -o foo/"\$\{newallargs\[\@\]\}" -L$UP{RIOT_LIB_PATH} \$CXXMPICH -lriot $FUNCTION_WRAPPERS/g;
...
#!/bin/bash
#
# Auto-generated mpicxx script from ldplfs-gen-cxx.pl
#
#
# Internal variables
...
Usage: $PROGRAM_NAME <stock mpicxx> --output <modified mpicxx>
